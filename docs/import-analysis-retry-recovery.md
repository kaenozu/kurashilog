...
The regression test exercises the complete sequence: first attempt commits source rows and fails during analysis; the second attempt sees a no-op source-key upsert, rebuilds analysis, and then completes without duplicating source records.
